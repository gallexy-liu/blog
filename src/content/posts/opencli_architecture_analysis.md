...
flowchart TB
    subgraph CLI["CLI 进程"]
        A["main.ts<br/>入口与参数解析"] --> B["cli.ts<br/>命令路由"]
        B --> C["execution.ts<br/>执行引擎"]
        C --> D["runtime.ts<br/>浏览器工厂"]
        C --> E["pipeline/executor.ts<br/>声明式管道"]
    end

    subgraph Daemon["Daemon 进程"]
        F["HTTP + WebSocket<br/>127.0.0.1:19825"]
    end

    subgraph Extension["Chrome Extension"]
        G["background.ts<br/>MV3 Service Worker"]
        H["cdp.ts<br/>chrome.debugger 桥接"]
    end
...
```mermaid
flowchart LR
    subgraph BrowserBridge["BrowserBridge：常规网站"]
        A1["CLI"] -->|"HTTP"| A2["Daemon"]
        A2 -->|"WebSocket"| A3["Chrome Extension"]
        A3 -->|"chrome.debugger"| A4["用户 Chrome 标签页"]
    end

    subgraph CDPBridge["CDPBridge：Electron / 调试端口"]
        B1["CLI"] -->|"WebSocket CDP"| B2["Electron / Chromium Endpoint"]
    end

    BrowserBridge --> I["统一 IPage 接口"]
    CDPBridge --> I
```
...
sequenceDiagram
    ...
    User->>CLI: opencli twitter trending
    CLI->>Exec: 解析命令并加载适配器
    Exec->>Daemon: GET /status
    Daemon-->>Exec: ready
    Exec->>Daemon: POST /command navigate
    Daemon->>Ext: WebSocket 转发
    Ext->>Chrome: tabs.create / debugger.attach
    Ext-->>Daemon: navigate ok
    Exec->>Daemon: POST /command exec
    Daemon->>Ext: WebSocket 转发
    Ext->>Chrome: Runtime.evaluate
    Chrome-->>Ext: 返回页面数据
    Ext-->>Daemon: result
    Daemon-->>Exec: HTTP response
    Exec-->>User: 输出 JSON / table
```
...
flowchart TB
    subgraph Workspaces["工作空间"]
        W1["site:twitter:uuid<br/>临时工作空间"]
        W2["browser:default<br/>持久工作空间"]
        W3["bound:research<br/>绑定工作空间"]
    end

    subgraph Leases["标签页租约"]
        L1["Ephemeral<br/>命令结束回收"]
        L2["Persistent<br/>空闲后回收"]
        L3["Bound<br/>借用用户标签页"]
    end

    subgraph ChromeTabs["Chrome 标签页"]
        T1["自动化容器标签页"]
        T2["自动化容器标签页"]
        T3["用户已有标签页"]
    end
